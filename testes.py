...
from classes import all_sprites, all_bullets, all_moedas, lasersprite, raposasprite, bobsprite, voando, Barry  # Importa todas as classes necessárias para o jogo do arquivo classes

from funcoes import criar_laser, criar_moedas, criar_raposa, criar_bob  # Importa as funções necessárias para o jogo do arquivo funcoes

pygame.mixer.music.play(loops=-1)  # Toca a música de fundo do jogo em loop



while GAME:  # Loop principal do jogo enquanto a variável GAME for True
    if not game_started:  # Se o jogo ainda não começou
        ...
        window.blit(imagens["logo"], (WIDTH / 2 - imagens["logo"].get_width() / 2, HEIGHT / 2 - imagens["logo"].get_height() + 100))  # Desenha o logo do jogo centralizado
        window.blit(textos["texto_renderizado1"], (WIDTH // 2 - textos["texto_renderizado1"].get_width() // 2, textos["posicao_y_linha1"] + 200))  # Desenha o primeiro texto na tela
        window.blit(textos["texto_renderizado2"], (WIDTH // 2 - textos["texto_renderizado2"].get_width() // 2, textos["posicao_y_linha2"] + 220))  # Desenha o segundo texto na tela
